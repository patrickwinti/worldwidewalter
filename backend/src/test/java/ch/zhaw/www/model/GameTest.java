...
class GameTest {
    ...
    void testGameState_WaitingForSelections() {
        ...
        assertNotNull(round);
        game.getActivePlayers().forEach((s, player) ->round.getPropositions().put(s, new Proposition(UUID.randomUUID().toString(), List.of("Walter " + player.getId()))));
        ...
        game.getCurrentRound().getPropositions().put(player.getId(), new Proposition(UUID.randomUUID().toString(), List.of("Walter " + player.getId())));
