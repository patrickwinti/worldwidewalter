...
class GameControllerTest {
    ...
    @Test
    void testStartNextRound_404() throws Exception {
        when(gameService.startNextRound(any())).thenThrow(new GameError.NotFoundException(GAME_ID));
        mvc.perform(MockMvcRequestBuilders.post("/api/games/{gameId}/rounds", GAME_ID))
                .andExpect(status().isNotFound());
        verify(gameService).startNextRound(GAME_ID);
    }
    ...
    @Test
    void testStartNextRound_409() throws Exception {
        when(gameService.startNextRound(any())).thenThrow(new GameError.NotEnoughPlayersException());
        mvc.perform(MockMvcRequestBuilders.post("/api/games/{gameId}/rounds", GAME_ID))
                .andExpect(status().isConflict());
        verify(gameService).startNextRound(GAME_ID);
    }
